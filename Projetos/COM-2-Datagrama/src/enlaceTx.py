...
class TX(object):
    ...
    def buildDataPackage(self, data):
        print("len payload: ", len(data))
        head = (len(data)).to_bytes(5, byteorder='big')
        payload = data
        eop = (888888888888).to_bytes(5, byteorder='big')
        pack = head + payload + eop
        overhead = len(pack)/len(payload)
        troughput = (len(payload)/len(pack))*115200

        print("__________________________________________________")
        print("Overhead: ", "%.5f" % overhead)
        print("Troughput: ", "%.3f" % troughput, "bits de payload/seg")
        print("Tempo esperado da transmissão: ", "%.2f" %((len(pack)/(115200/8))), "segundos")
        print("__________________________________________________")
    ...
    def sendBuffer(self, data):
        ...
        self.transLen = 0
        self.buffer = self.buildDataPackage(data)
        self.threadMutex = True
